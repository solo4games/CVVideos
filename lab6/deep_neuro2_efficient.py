...
from tensorflow.keras.applications import InceptionV3  # Предобученная модель InceptionV3
...
class FireClassifierInception:
    """
    Класс FireClassifierInception для классификации изображений на наличие или отсутствие пожара.
    Использует предобученную модель InceptionV3 для извлечения признаков.
    ...
    def __init__(self, model_path=None, input_shape=(128, 128, 3)):
        ...
        иначе создаёт новую модель на основе предобученной InceptionV3.
        ...
        if model_path and os.path.exists(model_path):
            ...
            # Загружаем предобученную InceptionV3 без верхних слоёв
            base_model = InceptionV3(weights='imagenet', include_top=False, input_shape=input_shape)
            base_model.trainable = False  # Замораживаем базовую модель, чтобы она не обновлялась при обучении

            # Создаём последовательную модель с пользовательскими слоями поверх InceptionV3
            self.model = Sequential([
                base_model,  # Базовая модель InceptionV3
    ...
    def train(self, train_path, val_path, save_path='fire_classifier_inception_model.h5'):
        """
...
classifier = FireClassifierInception()  # Создаём экземпляр классификатора
...
# classifier.train(train_path, val_path, save_path='fire_classifier_inception_model.h5')

# Пример использования сохранённой модели для предсказания
loaded_classifier = FireClassifierInception(model_path='fire_classifier_inception_model.h5')  # Загружаем сохранённую модель
