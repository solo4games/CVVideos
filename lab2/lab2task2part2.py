...
# Функция для создания идеального высокочастотного фильтра
def ideal_highpass_filter(shape, cutoff):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if distance <= cutoff:
                mask[i, j] = 0  # Низкие частоты обрезаются
    return mask

# Функция для применения идеального высокочастотного фильтра
def apply_ideal_highpass_filter(image, cutoff):
    # Преобразование изображения в частотную область
    dft = cv2.dft(np.float32(image), flags=cv2.DFT_COMPLEX_OUTPUT)
    dft_shifted = np.fft.fftshift(dft)

    # Создание маски для высокочастотной фильтрации
    mask = ideal_highpass_filter(image.shape, cutoff)
...
ideal_filtered_image = apply_ideal_highpass_filter(image, cutoff)
...
plt.title(f'Ideal Highpass Filter (cutoff={cutoff})')
